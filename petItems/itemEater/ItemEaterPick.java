...
public class ItemEaterPick extends ItemPickaxe {
	 ...
	 * */
	public ItemEaterPick(int par1, EnumToolMaterial par3EnumToolMaterial, Block[] eatAble ) {
			...
			if(this.hasApropriateBlock(player)){
				...
				if((this.getMaxDamage() - is.getItemDamage()) < this.getMaxDamage()){
					...
					if(consumedBlock != null){
						if(consumedBlock.equals(Block.oreIron)||consumedBlock.equals(Block.oreGold))
							addXp =5;
						if(consumedBlock.equals(Block.blockRedstone))
							addXp =15;
						if(consumedBlock.equals(Block.oreLapis))
							addXp =50;
					}

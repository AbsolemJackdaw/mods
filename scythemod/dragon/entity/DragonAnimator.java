...
import scythemod.dragon.CircularBuffer;
import scythemod.dragon.MathX;
import scythemod.dragon.ModelPart;
import scythemod.dragon.Spline;
import scythemod.dragon.ThirdPersonCameraAccessor;
import scythemod.dragon.TickFloat;
...
public class DragonAnimator {
    ...
    // helpers
    private ThirdPersonCameraAccessor tpca = new ThirdPersonCameraAccessor();
    ...
    public void update() {
        ...
        walkTimer.add(walkFlag ? 0.1f : -0.1f);
        ...
        speedTimer.add(speedFlag ? 0.05f : -0.05f);
        
        // update trailers
        yTrail.update(entity.posY - entity.yOffset);
        yawTrail.update(-entity.renderYawOffset);
        ...
        if (player.ridingEntity == entity) {
            float defaultDist = tpca.getDefaultThirdPersonDistance();
            tpca.setThirdPersonDistance(defaultDist + (1 - groundTimer.get()) * defaultDist);
        }
    }
    ...
    protected void animHeadAndNeck(DragonModel model) {
        ...
        float health = entity.getDragonHealth() / (float) entity.func_110138_aP();
        float neckSize;
    ...
    protected void animTail(DragonModel model) {
        ...
        for (int i = 0; i < model.tailProxy.length; i++) {
            ...
            model.tail.rotateAngleX += MathX.toRadians(MathX.normalizeAngle(pitchOfs));
            model.tail.rotateAngleX -= (1 - speed) * vertMulti * 2;
            model.tail.rotateAngleY += MathX.toRadians(MathX.normalizeAngle(yawOfs + 180));
    ...
    private void splineArrays(float x, boolean shift, float[] result, float[]... knots) {
        // uncomment to disable interpolation
//        if (true) {
//            if (shift) {
//                System.arraycopy(knots[(int) (x + 1) % knots.length], 0, result, 0, knots.length);
//            } else {
//                System.arraycopy(knots[(int) x % knots.length], 0, result, 0, knots.length);
//            }
//            return;
//        }
        
        int i1 = (int) x % knots.length;
        int i2 = (i1 + 1) % knots.length;
        int i3 = (i1 + 2) % knots.length;
        
        float[] a1 = knots[i1];
        float[] a2 = knots[i2];
        float[] a3 = knots[i3];
        
        float xn = x % knots.length - i1;

...
public class ItemHaloDeath extends Item {
	...
	{  
		...
		{ 
			if (par1.inventory.hasItem(BaseScytheFile.DedHalo.itemID) && !(par1.inventory.hasItem(BaseScytheFile.GodHalo.itemID))&& par2World.provider.isHellWorld == false)
			{
				par1.landMovementFactor = par1.capabilities.getWalkSpeed()*3;
				((EntityLiving)par3Entity).addPotionEffect(new PotionEffect(Potion.fireResistance.id, 60, 5));
			...
			else if( !par1.inventory.hasItem(BaseScytheFile.GodHalo.itemID) && (par1.inventory.hasItem(BaseScytheFile.DedHalo.itemID)) && par2World.provider.isHellWorld == true)
			{

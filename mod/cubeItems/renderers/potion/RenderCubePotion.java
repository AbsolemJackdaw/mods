...
public class RenderCubePotion extends RenderCubeInterface {
	...
	ModelCubeWorld potion_base_greyScale = new ModelCubeWorld(
			ModelCubeWorld.class
					.getResourceAsStream("/assets/subaraki/cubeModels/bottle_bottom_greyscale.cub"));
	ModelCubeWorld potion_base_empty = new ModelCubeWorld(
			ModelCubeWorld.class
					.getResourceAsStream("/assets/subaraki/cubeModels/bottle_bottom_empty.cub"));
	ModelCubeWorld potion_base_water = new ModelCubeWorld(
			ModelCubeWorld.class
					.getResourceAsStream("/assets/subaraki/cubeModels/bottle_bottom_water.cub"));
	ModelCubeWorld potion_top_splash = new ModelCubeWorld(
			ModelCubeWorld.class
					.getResourceAsStream("/assets/subaraki/cubeModels/bottle_top_splash.cub"));
	ModelCubeWorld potion_top_regular = new ModelCubeWorld(
			ModelCubeWorld.class
	...
	public void postSpecials(ItemStack item, ModelCubeWorld model,
		...
		if ((type == POTION) && (item.getItemDamage() > 0)) {
			int color = ((ItemPotion) item.getItem()).getColorFromDamage(item
					.getItemDamage());
		...
		if (isColored) {
			...
			// GL11.glEnable(GL11.GL_BLEND);
			// this.blockrender.renderStandardBlock(Block.dirt, (int)d, (int)d1,
			// (int)d2);
			// GL11.glColor3f(r, g, b);
			// this.blockrender.renderBlockAsItem(Block.blocksList[te.itemToImbue.itemID],
			// te.itemToImbue.getItemDamage(), f3);
